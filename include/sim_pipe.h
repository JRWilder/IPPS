...
typedef enum {LW, SW, ADD, ADDI, SUB, SUBI, XOR, BEQZ, BNEZ, BLTZ, BGTZ, BLEZ, BGEZ, JUMP, EOP, NOP, NONE = UNDEFINED} opcode_t;
typedef enum {PC, NPC, IR, A, B, IMM, COND, ALU_OUTPUT, LMD} sp_register_t;
typedef enum {IF, ID, EXE, MEM, WB} stage_t;
typedef enum {IF_ID, ID_EXE, EXE_MEM, MEM_WB} pipeline_register_t;
...
private:
  ...
  unsigned instr_base_address;
	...
	sp_register pipeline_register[NUM_STAGES-1];
	unsigned gp_register[NUM_GP_REGISTERS];
	double clock_cycle_count;
	double instr_exec;
	unsigned pc;
	unsigned stalls;
	unsigned IF_stalls;
...
public:
	...
	*/
	...
	void MEM_stage(void);
	void WB_stage(void);
	bool data_haz_check(void);
	bool cont_haz_check(void);
